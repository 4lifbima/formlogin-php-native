...
if (isset($_POST['login'])){
  ...
  if (password_verify($pass, $cari['password'])) {
    $_SESSION['userid'] = $cari['id'];
    $_SESSION['username'] = $cari['username'];
    $_SESSION['log'] = 'login';

  if($cari){
    echo '<script>alert("anda berhasil Login '.$cari['username'].'");window.location = "index.php"</script>';
  } else{
    echo '<script>alert("Data Yang Anda msukan salah Tahede");history.go(-1);</script>';
  }
} else{
  echo '<script>alert("Data Yang Anda msukan salah Tahede");history.go(-1);</script>';
}
          ...
          <div class="card-body">
            ...
            <div class="app-brand justify-content-center">
              <a href="index.html" class="app-brand-link gap-2">
                <span class="app-brand-text demo text-body fw-bolder">FORM LOGIN</span>
              </a>
            </div>
            <!-- /Logo -->
            <form method="POST">
